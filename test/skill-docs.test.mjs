...
// Paths a reader is expected to open: absolute repo paths, home-relative install
// paths, and doc-relative `references/x.md` / `presets/x/SKILL.md` links.
function referencedPaths(text, docPath) {
  const out = new Set();
  const docDir = path.dirname(docPath);
  // Repo-absolute paths are remapped onto the checkout under test so the suite is hermetic.
  for (const m of text.matchAll(/\/Users\/yeongyu\/local-workspaces\/OmOWright\/([A-Za-z0-9_./-]+)/g)) out.add(path.join(repoRoot, m[1]));
