...
test("legacy Aside class names are not public exports", () => {
  for (const name of ["AsidePage", "AsideMouse", "AsideKeyboard"]) {
    assert.equal(name in publicApi, false, `legacy export remains: ${name}`);
  }
});

test("runtime artifacts use the OmOWright namespace", () => {
  assert.match(coreSource, /"omowright-artifacts"/);
  assert.doesNotMatch(coreSource, /"asidewright-artifacts"/);
});
